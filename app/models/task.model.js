...
Task.updateByTask = (p, result) => {
    sql.query(
      "UPDATE tbl_priority_task SET creator_id = ?,project_id = ?,task_name = ?, priority = ?, planned_start_date = ?,planned_end_date = ?,actual_start_date = ?,actual_end_date = ?,description = ?,hourly_rate = ?,is_add_all = ?, is_active=? WHERE task_id = ?",
      [ p.creator_id,p.project_id,p.task_name,p.priority, p.planned_start_date, p.planned_end_date,p.actual_start_date,p.actual_end_date,p.description,p.hourly_rate,p.is_add_all,p.is_active, p.task_id], (err, res) => {
        if (err) {
          console.log("error: ", err);
          result(null, err);
          return;
